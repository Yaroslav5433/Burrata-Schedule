...
async def get_schedule(department: str, db: AsyncSession = Depends(get_db)):
    schedule = await db_req.get_all_users_saved_shifts(db, requested_position = department, week_dates = get_next_week_dates())
